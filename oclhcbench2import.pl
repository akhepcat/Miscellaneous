...
while (<>) {
	...
	if (m/^\*,999999,System Info,\d,(.*);(.*);(.*);(.*);(.*)/) {
		...
		$_ = $device{$dev}{"cpu"};
		if (m/pthread/i || m/cpu/i || m/processor/i || m/cortex/i) {
			$DEV="CPU";
			$device{$dev}{"cpu"} =~ s/pthread-//g;
			$device{$dev}{"cpu"} =~ s/cortex-/ARM /g;
			$device{$dev}{"cpu"} =~ s/ \w+-Core Processor//g;
...
for $d (keys %device) {
	...
	for $m (sort { $a <=> $b } keys %{$device{$dev}}) {
		...
		printf("%d,%s,%s,%s,%s,%.0f,%.1f %s,%s\n",
			$m, $device{$d}{$m}{"desc"}, $host, $device{$d}{"dev"}, $device{$d}{"cpu"}, $ores, $res, $unit, $device{$d}{$m}{"notes"});
	}
}
